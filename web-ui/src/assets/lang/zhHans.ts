...
const zhHans = {
  ...
  settings: {
    ...
    resourceUpdate: {
      ...
      progress: {
        remoteHeadNotFound: '无法获取远端 HEAD',
        gitNotFound: '未找到 git 可执行文件',
      },
  ...
  backend: {
    ...
    device: {
      windows: {
        ...
        unavailable: {
          ...
          unknown: 'PC 模式当前不可用。',
          init_failed: 'Windows 设备初始化失败：{message}',
        },
      },
      mac: {
        available: '',
        unavailable: {
          ...
          port_not_configured: 'MacPlayTools 端口未配置。请在 PlayCover 中启动游戏后，从窗口标题栏获取 [localhost:端口号] 并填入配置。',
          connect_failed: '无法连接到 MacPlayTools ({host}:{port})。请确认 PlayCover 中的游戏已启动且 MaaTools 已启用。',
          init_failed: 'MacPlayTools 设备初始化失败：{message}',
        },
      },
    },
    task: {
      ...
      gameNotInForeground: '游戏未在前台运行，请手动切回游戏后重试',
      gameNotStarted: '游戏未启动，请先手动启动游戏后重试',
      suspendSwitchToAlbum: '任务已挂起，请手动切换到图鉴页面',
      tabBarNotFound: '无法找到TabBar，刷新失败',
      requiredButtonNotFound: '无法找到任务所需的按钮，刷新失败',
      cardAttributeSwitchNotFound: '无法找到卡属性切换按钮，刷新失败',
      idolSwitchNotFound: '无法找到偶像切换按钮，刷新失败',
      startIdolCardCLIP: '开始偶像卡 CLIP 学习，将按底部卡条逐张遍历',
      startSupportCardCLIP: '开始支援卡 CLIP 学习，请确保已进入支援卡列表页面',
      noEnhancementConfigured: '未配置任何需要强化的品级',
      clipNotInitialized: 'CLIP 服务未初始化，无法学习偶像卡 CLIP',
      notOnIdolCardPage: '当前不在偶像卡养成页面，未检测到 PRODUCT_CARD_SELECTED',
      externalPageRecoveryFailed: '外页恢复失败：已回到主页并打开 Produce，但未命中未完成培育再开弹窗',
      produceFinishTimeout: '培育收尾失败：推进结果链超时仍未回到主页',
      consecutiveUnknownsExceeded: '培育主循环: 连续无法识别画面阈值超出',
      noHandlerPause: '培育主循环: 当前页面无可用 handler，已暂停等待分析 (phase={phase}, position={position})',
      maxGameplayLoopsExceeded: '培育主循环: 达到最大循环次数 {max_loops}',
      supportCardSelectionFailed: '未能完成支援卡选择',
      unknownSupportCardMode: '未知支援卡编成模式: {mode}',
      idolCardInfoNotFound: '无法获取偶像卡信息',
      trainingInfoCloseButtonNotFound: '未找到育成情報关闭按钮，也无法推导蒙层关闭点',
      candidateListEmpty: '候选列表为空',
      combatPowerAnchorNotFound: '找不到[総合力合計]锚点',
    },
    gameAsset: {
    ...
    adb: {
      ...
      unavailable: 'Android 设备不可用。',
      touchServiceUnavailable: '点击失败：触摸服务不可用',
      reconnectFailed: '重新连接 ADB 设备失败',
    },
    gameUtils: {
      modalHeaderTopEmpty: '无法读取模态标题位置：modal 为空',
      modalMissingHeaderBox: '模态 \'{title}\' 缺少 header_box，无法判断稳定性',
      modalHeaderMissingCoords: '模态 \'{title}\' 的 header_box 缺少 y/cy 坐标，无法判断稳定性',
      modalSignatureEmpty: '无法构建模态签名：modal 为空',
      modalSignatureMissingHeader: '模态 \'{title}\' 缺少 header_box，无法构建签名',
      modalHeaderNoCenter: '模态 \'{title}\' 的 header_box 无法读取中心点',
      modalNoActionButtons: '模态 \'{title}\' 缺少可用的操作按钮，无法构建签名',
    },
  },
  config: {
